...
def acharVerticeInicial(ciclo, grafoRestante):
	...
	#print("Achando o novo verticeInicio")
	#print("ciclo", ciclo)
	for i in range(0, len(ciclo)):
...
def acharEulerCiclo(listAdj):
	...
	#print("grafoRestante", grafoRestante)
	...
	while len(grafoRestante) != 0:
		...
		#print("#######verticeInici:", verticeInicio)
		...
		while extremidadeAresta != indiceVerticeInicio:
			...
			#print("ExtremidadeAresta", extremidadeAresta)
		...
		#print("ciclo após incluir o novo cilco ciclo achado",ciclo)
	...
	for item in items[1:]:
		...
		if restos:
			for fim in duplasUnicas(restos):
				yield from ([par] + fim for fim in duplasUnicas(restos))
		else:
			yield [par]
...
#gera um grafo completo aleatório de N vértices
...
#gera un grafo aleatorio de N vértices
...
def resolve(grafo):
	...
	tipo = verificaGrafoEuleriano(listaAdj)
	print("VerificaGrafoEuleriano(listAdj):", verificaGrafoEuleriano(listaAdj))
	...
	if tipo != 1:
		...
		print("acharEulerCiclo", acharEulerCiclo(listaAdj))

		f =  open("aleatoryGraph.txt", "w")
		f.write(str(listaAdj))
		f.close()
...
def main():
	...
	print("", t2, t1)
	print("", N, numeroAresta(grafo), t2 - t1)
